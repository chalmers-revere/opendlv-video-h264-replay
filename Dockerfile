...
# Part to build opendlv-video-h264-replay.
...
# Part to deploy opendlv-video-h264-replay.
FROM ubuntu:20.04
MAINTAINER Christian Berger "christian.berger@gu.se"
...
WORKDIR /usr/bin
COPY --from=builder /tmp/bin/opendlv-video-h264-replay .
ENTRYPOINT ["/usr/bin/opendlv-video-h264-replay"]
